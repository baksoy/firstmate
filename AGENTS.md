...
config/secondmate-harness  harness the PRIMARY uses to launch SECONDMATE agents, optionally followed by a model and effort token on the same line ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates). An optional per-mate sibling config/secondmate-harness.<id> uses the same one-line grammar and, when present, is resolved above this global file for that one mate so its runtime survives every respawn (docs/configuration.md)
