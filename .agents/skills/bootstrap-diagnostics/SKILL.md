...
description: >-
  Load on any actionable session-start or standalone bootstrap/network diagnostic, including every FLEET_SYNC line; before classifying FLEET_SYNC as harmless, inspect the named clone's branch and registry state and record why no repair is required.
...
- `FLEET_SYNC: <repo>: skipped: <reason>` - bootstrap continued, but do not classify the skip as harmless from its label alone.
  Inspect the named clone's current branch and the project registry, then record the concrete reason no repair is required; otherwise treat the skip as needing attention.
  A skip can report offline, no origin, local-only, no local main, or the bounded fleet-refresh timeout (`FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT`, or a fleet-size-aware default with a 20 second floor); a timeout never blocks startup by itself.
