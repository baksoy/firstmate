...
`bin/fm-spawn.sh --secondmate` launches it through the secondmate harness path, resolving the mate's own per-mate pin `config/secondmate-harness.<id>` -> the global `config/secondmate-harness` -> `config/crew-harness` -> the primary's own harness unless an explicit per-spawn harness override is passed.
...
When two secondmates must run different runtimes, a per-mate file `config/secondmate-harness.<id>` pins that one mate above the shared global file; it reuses the identical `<harness> [<model>] [<effort>]` grammar and parser, so `default`/absent still defers downward and a bare harness still means harness-only (full grammar and precedence owned by [`docs/configuration.md`](../../../docs/configuration.md)).
Pass the mate's id as the trailing argument to the resolver verbs (`bin/fm-harness.sh secondmate <id>`, `secondmate-model <id>`, `secondmate-effort <id>`) to read it; `bin/fm-spawn.sh --secondmate`, `bin/fm-control.sh <id> relaunch`, the `/updatefirstmate` restart pass, and liveness relaunch all key by the mate's id, so a per-mate runtime survives every respawn without reverting to the global default.
With no per-mate file present, resolution is byte-for-byte the global-file behavior above, so distinct-runtime mates are opt-in per mate.
...
To move a live LOCAL secondmate onto a newly pinned harness, model, or effort without a full recovery, set `config/secondmate-harness` (or that one mate's `config/secondmate-harness.<id>` to move it alone) and then relaunch it with `bin/fm-control.sh <id> relaunch`, which re-resolves the pin keyed to the mate's id, stops the agent, and launches the replacement in the same home ([`docs/agent-control.md`](../../../docs/agent-control.md)).
